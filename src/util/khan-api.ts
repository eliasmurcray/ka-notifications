...
export async function khanApiFetch(queryName: string, authToken: string, variables: KhanAPIVariables = {}): Promise<Response> {
	...
	const requestInit: RequestInit = {
		...
		body: `{"operationName":"${queryName}","query":"${query}","variables":${stringifiedVariables}}`,
	...
	if (response.status === 400) {
		console.warn(`The query for operation "${queryName}" is no longer in the safelist. Attempting to fetch the latest version from the safelist...`);

		const latestQuery = query.startsWith("mutation") ? await getLatestMutation(queryName) : await getLatestQuery(queryName);

		if (!latestQuery) {
			throw new Error(`The query for operation "${queryName}" was not found in the safelist`);
		}

		requestInit.body = `{"operationName":"${queryName}","query":"${query}","variables":${stringifiedVariables}}`;
