 ...
 */
export function graphQLFetch(queryName: "AddFeedbackToDiscussion" | "clearBrandNewNotifications" | "feedbackQuery" | "getFeedbackRepliesPage" | "getFullUserProfile" | "getNotificationsForUser", kaas: string, variables: graphQLVariables = {}): Promise<Response> {
  return new Promise((resolve, reject) => {
    // Implement fastly phrase to match safelist regex, pushing ratelimit to 100 tps for any Khan Academy GraphQL call
    fetch("https://www.khanacademy.org/api/internal/graphql/" + queryName + "?/_fastly/", {
      method: "POST",
      headers: {
        "X-KA-fkey": "a",
