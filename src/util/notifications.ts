...
async function addFeedback (feedbackType: RequestType, responseType: ResponseType, topicId: string, qaExpandKey: string, textContent: string, expandType: string, focusKind: string = "scratchpad"): Promise<void | Response> {
...
export async function* createNotificationsGenerator (cursor: string = ""):  AsyncGenerator<Notification[], Notification[]>{
	let complete = false;
	for(;!complete;) {
