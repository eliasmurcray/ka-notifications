...
window.fetch = async function(
  ...
  if (
    ...
    return await (proxiedFetch(request, requestInit).catch(Math.abs) as Promise<
      Response
    >);
  }
 ...
 */
const qaExpandType = new URLSearchParams(window.location.search).get(
  "qa_expand_type"
);
let i = 0;
