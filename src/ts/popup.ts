...
import {
  createNotificationsGenerator,
  createNotificationHTMLDivElement,
  renderFromCache,
...
const notificationsGenerator = createNotificationsGenerator(
  CACHED_DATA?.cursor ?? ""
);
...
const markReadLoading = document.getElementById(
  "mark-read-loading"
) as HTMLDivElement;
const sortCommentsSelect = document.getElementById(
  "sort-comments"
...
async function loadNotifications(): Promise<void> {
  ...
  if (result.done === true) {
    ...
    if (hasNotifications === true) {
      ...
      notificationContent.append("You have no notifications.");
  ...
  if (result.value.error === "No fkey cookie found.") {
    ...
    notificationContent.append(
      "Your fkey cookie has expired. You need to ",
      hyperlink,
  ...
  if (notifications === undefined) {
    ...
    notificationsContainer.append(
      '<div class="notification"><div class="notification-header"><img class="notification-author--avatar" src="32.png"><h3 class="notification-author--nickname">KA Notifications</h3><span class="notification-date">0s ago</span></div><p class="notification-content">You have no notifications.</p></div>'
    );
    return;
  }
...
function markAllRead(): Promise<ClearBrandNewNotificationsResponse> {
  return new Promise((resolve, reject) => {
    getChromeFkey()
      ...
      .then(async (response) => {
        const json =
          (await response.json()) as ClearBrandNewNotificationsResponse;
        if (
          json.data.clearBrandNewNotifications.error?.code === "UNAUTHORIZED"
        ) {
          reject();
        } else {
          resolve(json);
